...
class ToTensor(object):
    ...
    def __call__(self, image, mask=None):
        ...
        if image is None:
            print("None")
            return image
        # image1 = paddle.vision.transforms.to_tensor(image)
        # image = np.array(image)
        # mask = np.array(mask)
        # print(image.dtype,"image---------", image.shape,mask.shape)
        
        if isinstance(image, np.ndarray) and image.dtype != np.uint8:
            if self.image_keep_255:
                return to_tensor(image)
            else:
                return to_tensor(image).div(255.)

        if self.image_keep_255:
            # image = paddle.to_tensor(image/255,dtype="float32").transpose([2,0,1])
            # image.save('temp.png')
      
            image = np.array(image, dtype="float32")
            image = np.transpose(image, [2,0,1])
            # # print("before",image.dtype, image is None)
            image = paddle.to_tensor(image)
          
            # image = paddle.transpose(image,[2,0,1])
        ...
        if mask is None:
            ...
            return image, paddle.to_tensor(np.array(mask, copy=False))
