from simplecv1.util.logger import get_logger
...
def copy_bn_parameters(src: nn.BatchNorm2D, dst: nn.BatchNorm2D):
    if dst.affine:
        dst.weight.data = src.weight.data.clone().detach()
        dst.bias.data = src.bias.data.clone().detach()
...
def copy_weight_bias(src: nn.Layer, dst: nn.Layer):
