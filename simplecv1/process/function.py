...
def th_mean_std_normalize(image, mean=(123.675, 116.28, 103.53), std=(58.395, 57.12, 57.375)):
    ...
    if image.dim() == 3:
        ...
        raise dim_error
    shape[idx] = -1
    mean = paddle.to_tensor(mean, requires_grad=False).reshape(*shape)
    std = paddle.to_tensor(std, requires_grad=False).reshape(*shape)
...
def th_divisible_pad(tensor, size_divisor: int, mode='constant', value=0):
    ...
    pad = [0, nwidth - width, 0, nheight - height] 
#     print(pad,"pad")
