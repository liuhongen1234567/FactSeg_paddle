...
class MultiResolutionFusion(nn.Layer):
...
class ChainedResidualPool(nn.Layer):
...
class ChainedResidualPoolImproved(nn.Layer):
    def __init__(self, feats):
        super().__init__()
...
class BaseRefineNetBlock(nn.Layer):
...
class RefineNetBlockImprovedPooling(nn.Layer):
    def __init__(self, features, *shapes):
        super().__init__(features, ResidualConvUnit, MultiResolutionFusion,
